...
int main(int argc, char *argv[])
{
    ...
    char curl_easy_setopt_data[PACKET_MAX_SIZE + 20];

    while (fscanf(fp, "%s", line_buf) != EOF) {
        CURL *curl;
        CURLcode res = CURLE_OK;

        struct curl_slist *host = curl_slist_append(NULL, "::speedtest.selectel.ru");

        curl = curl_easy_init();
        if (curl) {
            sprintf(curl_easy_setopt_data, "https://%s/100MB", line_buf);
